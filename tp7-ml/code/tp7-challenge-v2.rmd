...
data_cat <- filtered_data %>% dplyr::mutate(circ_tronco_cm_cat = sapply(circ_tronco_cm, categorize_circ))
formula <- formula(inclinacion_peligrosa ~ circ_tronco_cm_cat + lat + seccion + diametro_tronco  + altura)

data_cat$inclinacion_peligrosa <- as.factor(data_cat$inclinacion_peligrosa)
cmodel <- train(
  formula,
  data = data_cat,
  trControl = ctrl,
...
test2 <- test %>% dplyr::mutate(circ_tronco_cm_cat = sapply(circ_tronco_cm, categorize_circ))
p <- predict(cmodel, test2)

r <- create_submission(p, test, "09_cv_k10")
```
```{r}
library(xgboost)
